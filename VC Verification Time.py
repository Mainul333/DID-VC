...
for i in range(VC_count):
    ...
    verification=verify(bytes.fromhex(issuer_pubkey),(str(data['@context'])+str(data['issuer'])+str(data['subject'])).strip(),bytes.fromhex(issuer_signature))
    end_time=time.time()
    elapsed_time=end_time-start_time
    total_time=total_time+elapsed_time
    verification_time.append([i,elapsed_time])
    print("VC verification latency-",i,":",int(elapsed_time*1000)/1000," seconds")
with open('VC_Verification_Time.csv', 'w', encoding='UTF8', newline='') as f:
    writer = csv.writer(f)
...
print("-------------------------------------------------------------------------")
print("Average latency required to verify VC:",int(total_time*1000/VC_count)/1000," seconds")
while True:
    hold=1
