...
with open('DID.json', 'r') as file:
    data = json.load(file)

student_pubkey=data['student']['publicKey']['value']
issuer_pubkey=data['issuer']['publicKey']['value']
student_signature=data['authentication']['signature']
issuer_signature=data['proof']['signature']
DID_count=1000
total_time1=0
total_time2=0
autentication_time=[]
...
for i in range(DID_count):
    ...
    authentication=verify(bytes.fromhex(student_pubkey),str(data['student']).strip(),bytes.fromhex(student_signature))
    end_time1=time.time()
    elapsed_time1=end_time1-start_time1
    total_time1=total_time1+elapsed_time1
    autentication_time.append([i,elapsed_time1])
    start_time2=time.time()
    verification=verify(bytes.fromhex(issuer_pubkey),(str(data['@context'])+str(data['issuer'])+str(data['student'])+str(data['authentication'])).strip(),bytes.fromhex(issuer_signature))
    end_time2=time.time()
    elapsed_time2=end_time2-start_time2
    total_time2=total_time2+elapsed_time2
    verification_time.append([i,elapsed_time2])
    print("-------------------------------------------------------------------------")
    print("DID autentication latency-",i,":",int(elapsed_time1*1000)/1000," seconds")
    print("DID verification latency-",i,":",int(elapsed_time2*1000)/1000," seconds")
with open('DID_Authentication_Time.csv', 'w', encoding='UTF8', newline='') as f:
    writer = csv.writer(f)
    ...
    # write multiple rows
    writer.writerows(autentication_time)
with open('DID_Verification_Time.csv', 'w', encoding='UTF8', newline='') as f:
    writer = csv.writer(f)
    ...
    # write multiple rows
    writer.writerows(verification_time)
print("-------------------------------------------------------------------------")
print("Average latency required to authenticate DID:",int(total_time1*1000/DID_count)/1000," seconds")
print("Average latency required to verify DID:",int(total_time2*1000/DID_count)/1000," seconds")
while True:
    hold=1
